...
function optimizeHTML(p) {
  return function(next) {
    ...
    log.info('Optimize HTML: %s ' + '[%s saved]'.yellow + ' (%dms)'.grey, p, saved + '%', Date.now() - start);
    next();
  };
}
...
function optimizeCSS(p) {
  return function(next) {
    ...
    fs.writeFileSync(p, result, 'utf8');
    var saved = parseInt((content.length - result.length) / content.length * 100, 10);
    log.info('Optimize CSS: %s ' + '[%s saved]'.yellow + ' (%dms)'.grey, p, saved + '%', Date.now() - start);
    next();
  };
}
...
function optimizeJS(p) {
  return function(next) {
    ...
    fs.writeFileSync(p, result, 'utf8');
    var saved = parseInt((content.length - result.length) / content.length * 100, 10);
    log.info('Optimize JS: %s ' + '[%s saved]'.yellow + ' (%dms)'.grey, p, saved + '%', Date.now() - start);
    next();
  };
}
  ...
  return function(next) {
    ...
    .optimize(
      function(err, file) {
        ...
        log.info('Optimize Image: %s ' + '[%s saved]'.yellow + ' (%dms)'.grey, p, saved + '%', Date.now() - start);
        next();
      });
  };
...
function optimize(args) {
  async.series([
    ...
    // 处理HTML
    function(next) {
      log.debug('Optimize HTML start');
      async.parallel(htmlList, function(err, result) {
        log.debug('Optimize HTML end');
        next();
      });
    },
    // 处理CSS
    function(next) {
      log.debug('Optimize CSS start');
      async.parallel(cssList, function(err, result) {
        log.debug('Optimize CSS end');
        next();
      });
    },
    // 处理JS
    function(next) {
      log.debug('Optimize JS start');
      async.parallel(jsList, function(err, result) {
        log.debug('Optimize JS end');
        next();
      });
    },
    // 处理Image
    function(next) {
      log.debug('Optimize Image start');
      async.parallel(imageList, function(err, result) {
        log.debug('Optimize Image end');
        next();
      });
    },
    // 处理结束
